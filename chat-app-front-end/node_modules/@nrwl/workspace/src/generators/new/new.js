...
function newGenerator(host, options) {
    return tslib_1.__awaiter(this, void 0, void 0, function* () {
        ...
        workspace_1.workspaceGenerator(host, workspaceOpts);
        ...
        host.listChanges().forEach((change) => {
            if (change.type !== 'DELETE') {
                host.rename(change.path, path_1.join(options.directory, change.path));
            }
        });
        return () => tslib_1.__awaiter(this, void 0, void 0, function* () {
            devkit_1.installPackagesTask(host, false, options.directory);
...
function addCloudDependencies(host, options) {
    if (options.nxCloud) {
        return devkit_1.addDependenciesToPackageJson(host, {}, { '@nrwl/nx-cloud': 'latest' });
    }
}
...
function addPresetDependencies(host, options) {
    ...
    return devkit_1.addDependenciesToPackageJson(host, dependencies, dev);
}
...
function setDefaultLinter(host, { linter, preset }) {
 ...
 */
function setESLintDefault(host) {
 ...
 */
function setTSLintDefault(host) {
    devkit_1.updateJson(host, devkit_1.getWorkspacePath(host), (json) => {
...
function setDefaultPackageManager(host, { packageManager }) {
    if (!packageManager) {
        return;
    }
    devkit_1.updateJson(host, devkit_1.getWorkspacePath(host), (json) => {
